...
const PatientMessageList = () => {
  const { patientMessageList } = mockData;
  const [searchTerm, setSearchTerm] = useState('');
  const [filter, setFilter] = useState('All');
  const [showPinnedOnly, setShowPinnedOnly] = useState(false);

  const filteredMessages = useMemo(() => {
    return patientMessageList.filter(message => {
  ...
  }, [patientMessageList, searchTerm, filter, showPinnedOnly]);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden max-w-md mx-auto">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 border-b border-gray-200 flex items-center justify-between">
        ...
        {sortedMessages.length === 0 ? (
          ...
          sortedMessages.map((message) => (
            <div 
              ...
              className="p-4 hover:bg-blue-50/50 transition-colors duration-200 group cursor-pointer flex items-center space-x-4"
            >
              {message.isPinned && (
                <Pin 
                  size={20} 
